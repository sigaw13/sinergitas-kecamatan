...
if (process.env.DATABASE_URL && process.env.NODE_ENV === 'production') {
  ...
  // Wrapper untuk kompatibilitas dengan SQLite API
  db = {
    get: (sql, params, callback) => {
      // Konversi ? ke $1, $2, dst untuk PostgreSQL
      const pgSql = sql.replace(/\?/g, (_, index) => `$${index + 1}`);
  ...
  // Initialize SQLite tables (kode yang sama seperti sebelumnya)
  sqliteDb.serialize(() => {
    // ... kode SQLite initialization ...
  });
}
